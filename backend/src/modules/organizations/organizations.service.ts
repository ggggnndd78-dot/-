...
export class OrganizationsService {
  ...
  private async getOrganizationForMember(
    userId: number,
  ...
  private ensureOwnerCanManageBranches(member: any) {
    if (
      ![
        "owner",
        "merchant_owner",
  ...
  private ensureMemberCanViewAuditLogs(member: any) {
    const isOwner = [
      "owner",
      "merchant_owner",
      "workshop_owner",
    ...
    const permissionCodes = Array.isArray(member.permissions)
      ? member.permissions.map((item: any) => String(item.permissionCode))
      : [];
  ...
  private readAuditMetadata(metadata: Prisma.JsonValue | null | undefined) {
  ...
  private mapBranch(branch: any) {
    return {
      ...
      business_hours: (branch.businessHours ?? []).map((item: any) => ({
  ...
  private async getOrganizationOwner(organizationId: number) {
  ...
  private arabicOrganizationType(type: string) {
    if (type === "MERCHANT") return "التاجر";
    if (type === "WORKSHOP") return "الورشة";
    if (type === "WAREHOUSE") return "المستودع";
    return "المنشأة";
  }

  private async notifyOwnerByReviewResult(input: {
    ...
    > = {
      APPROVED: {
        ...
        body:
          input.organizationType === "MERCHANT"
  ...
  private mapOrganization(organization: any) {
    return {
      ...
      merchant_profile: organization.merchantProfile
        ? {
            business_category_code:
              organization.merchantProfile.businessCategoryCode,
            average_preparation_minutes:
              organization.merchantProfile.averagePreparationMinutes,
            warranty_policy_text:
              organization.merchantProfile.warrantyPolicyText,
            return_policy_text: organization.merchantProfile.returnPolicyText,
            delivery_policy_text:
              organization.merchantProfile.deliveryPolicyText,
            min_order_amount:
              organization.merchantProfile.minOrderAmount?.toString(),
          }
        : null,
      workshop_profile: organization.workshopProfile
        ? {
            ...
            accepts_installation:
              organization.workshopProfile.acceptsInstallation,
            capacity_per_day: organization.workshopProfile.capacityPerDay,
            supports_emergency_service:
              organization.workshopProfile.supportsEmergencyService,
            default_diagnosis_fee:
              organization.workshopProfile.defaultDiagnosisFee?.toString(),
          }
        : null,
      branches: (organization.branches ?? []).map((branch: any) => ({
        id: branch.publicId,
        branch_name: branch.branchName,
        email: branch.email,
        phone: branch.phone,
      ...
      bank_accounts: (organization.bankAccounts ?? []).map((bank: any) => ({
        id: bank.id,
      ...
      verification_requests: (organization.verificationRequests ?? []).map(
        (request: any) => ({
          id: request.publicId,
    ...
    return {
      ...
      data: organizations.map((item) => ({
        ...
        latest_verification_status:
          item.organization.verificationRequests[0]?.status ?? null,
        branches_count: item.organization.branches.length,
      })),
    };
  }

  async detailForMember(userId: number, organizationPublicId: string) {
    const { organization } = await this.getOrganizationForMember(
      userId,
  ...
  async listAuditLogs(
    userId: number,
    organizationPublicId: string,
    query: { take?: number; skip?: number; action?: string },
  ) {
    const organization = await this.prisma.organization.findUnique({
      where: { publicId: organizationPublicId },
      include: {
    ...
    const memberUserIds = organization.members.map((item: any) => item.userId);
    ...
    const relevant = candidates.filter((log: any) => {
      ...
      return (
        organizationMarkers.includes(String(organization.id)) ||
  ...
  async listBranches(userId: number, organizationPublicId: string) {
    const { organization } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    );
  ...
  async updateBranch(
    ...
    const { organization, member } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    );
    ...
    const updated = await this.prisma.organizationBranch.update({
      ...
      data: {
        ...
        supportsInstallation:
          dto.supportsInstallation ?? branch.supportsInstallation,
        supportsMobileService:
          dto.supportsMobileService ?? branch.supportsMobileService,
      },
      include: { city: true, district: true, area: true, businessHours: true },
    });
    await this.audit.write({
  ...
  async setBranchClosure(
    userId: number,
    organizationPublicId: string,
    branchPublicId: string,
    dto: SetBranchClosureDto,
  ) {
    const { organization, member } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    ...
    const updated = await this.prisma.organizationBranch.update({
      ...
      data: {
        ...
        closedUntil:
          dto.temporarilyClosed && dto.closedUntil
  ...
  async upsertBranchBusinessHours(
    userId: number,
    organizationPublicId: string,
    branchPublicId: string,
    ...
    if (uniqueDays.size !== dto.items.length)
      throw new BadRequestException("Each day of week must appear once only");
    await this.prisma.$transaction(
      dto.items.map((item) =>
        this.prisma.branchBusinessHour.upsert({
          where: {
            branchId_dayOfWeek: {
              branchId: branch.id,
  ...
  async getBusinessHours(userId: number, organizationPublicId: string) {
    const { organization } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    );
    const headOffice =
      organization.branches.find((b) => b.isHeadOffice) ??
      organization.branches[0];
    if (!headOffice) {
      throw new NotFoundException("Organization has no branch yet");
    }
  ...
  async upsertMerchantProfile(
    ...
    if (organization.organizationType !== "MERCHANT") {
      throw new BadRequestException("This organization is not a merchant");
    }
  ...
  async upsertWorkshopProfile(
    userId: number,
    organizationPublicId: string,
    dto: UpsertWorkshopProfileDto,
  ) {
    const { organization } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    ...
    if (organization.organizationType !== "WORKSHOP") {
      throw new BadRequestException("This organization is not a workshop");
    }
  ...
  async listBankAccounts(userId: number, organizationPublicId: string) {
    const { organization } = await this.getOrganizationForMember(
      userId,
      organizationPublicId,
    );
  ...
  async submitVerificationRequest(
    ...
    if (existingOpenRequest) {
      throw new BadRequestException(
        "There is already an active verification request for this organization",
      );
    }
    ...
    const request = await this.prisma.$transaction(async (tx) => {
      ...
      await (tx as any).verificationStatusHistory.create({
        data: {
      ...
      if (dto.notes) {
        await (tx as any).verificationReviewNote.create({
          data: {
            verificationRequestId: created.id,
            organizationId: organization.id,
            actorUserId: userId,
  ...
  async getVerificationRequest(userId: number, requestPublicId: string) {
    ...
    const isMember = request.organization.members.some(
      (m) => m.userId === userId,
    );
    if (!isMember)
      throw new ForbiddenException(
    ...
    return {
      ...
      data: {
        ...
        documents: request.documents.map((doc) => ({
          ...
          file_size_bytes: (doc as any).fileSizeBytes ?? null,
          storage_provider: (doc as any).storageProvider ?? null,
          upload_status: (doc as any).uploadStatus ?? null,
          side: (doc as any).side ?? null,
          preview_base64: (doc as any).fileContentBase64 ?? null,
          notes: doc.notes,
        })),
        review_notes_history:
          (request as any).reviewNotesList?.map((note: any) => ({
            id: note.id,
            note_type: note.noteType,
            note: note.note,
            actor:
              note.actor?.displayName ?? note.actor?.phoneNormalized ?? null,
            created_at: note.createdAt,
          })) ?? [],
        status_history:
          (request as any).statusHistory?.map((history: any) => ({
            id: history.id,
  ...
  async addVerificationDocument(
    ...
    const documentEditableStatuses: string[] = [
      "DRAFT",
      "SUBMITTED",
      "PENDING_REVIEW",
      "UNDER_REVIEW",
    ...
    if (!documentEditableStatuses.includes(request.status)) {
      throw new BadRequestException(
        "Cannot add documents to this verification request in its current status",
      );
    }
    ...
    await (this.prisma as any).verificationReviewNote.create({
      data: {
        verificationRequestId: request.id,
        ...
        note:
          dto.notes ?? `Uploaded document ${dto.documentType}: ${dto.fileName}`,
      },
    });
    ...
    return {
      ...
      data: {
        ...
        storage_provider: (document as any).storageProvider ?? null,
        upload_status: (document as any).uploadStatus ?? null,
        side: (document as any).side ?? null,
      },
    };
  }

  async listVerificationRequestsForAdmin(userId: number, status?: string) {
  ...
  async getVerificationRequestForAdmin(
    userId: number,
    requestPublicId: string,
  ) {
    await this.assertAdmin(userId);
    ...
    return {
      ...
      data: {
        ...
        approval_actions: request.approvalActions.map((action) => ({
          ...
          notes: action.notes,
          acted_by:
            action.actedBy.displayName ?? action.actedBy.phoneNormalized,
  ...
  async reviewVerificationRequest(
    userId: number,
    requestPublicId: string,
    action: "approve" | "reject" | "require_documents" | "suspend",
    ...
    switch (action) {
      ...
      case "require_documents":
        ...
        break;
      case "suspend":
        requestStatus = "SUSPENDED" as any;
    ...
    if (!openStatuses.includes(request.status as any) && action !== "suspend") {
      throw new BadRequestException(
        "This verification request is not reviewable in its current status",
      );
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (action === "approve") {
        const roleCode =
          request.organization.organizationType === "MERCHANT"
            ? "merchant_owner"
